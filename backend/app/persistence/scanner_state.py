...
def _optional_utc(value: str | None) -> datetime | None:
    return _utc(datetime.fromisoformat(value)) if value is not None else None
...
def _context_payload(context: EvaluationContext) -> dict[str, Any]:
    return {
        ...
        "freshness": {
            key: format(value, "f") for key, value in context.freshness.items()
        },
    }
...
def _context_from_payload(payload: dict[str, Any]) -> EvaluationContext:
    return EvaluationContext(
        ...
        exchange_time=_utc(datetime.fromisoformat(payload["exchange_time"])),
        counts={str(key): int(value) for key, value in payload["counts"].items()},
        freshness={
            str(key): Decimal(str(value))
            for key, value in payload["freshness"].items()
        },
    )
...
class ScannerRuntimeStateStore:
    ...
    def save(self, scanner: ScannerService) -> None:
        payload = {
            ...
            "candidates": [
                ...
                for candidate in scanner._candidates.values()
            ],
            "candidate_contexts": {
            ...
            "terminal_history": [
                ...
                for (symbol, direction, setup), terminal_at in (
                    scanner._terminal_history.items()
                )
            ],
    ...
    def restore(self, scanner: ScannerService) -> bool:
        ...
        if int(row["schema_version"]) != _SCHEMA_VERSION:
            raise ScannerStateRecoveryError(
                "Unsupported scanner snapshot schema version"
            )
        try:
            ...
            scanner._runs = [
                ScannerRunSummary.model_validate(item) for item in payload["runs"]
            ]
            scanner._next_full_scan_at = _optional_utc(
                payload["next_full_scan_at"]
            )
            scanner._next_refresh_at = _optional_utc(payload["next_refresh_at"])
            scanner._last_refresh_boundary = _optional_utc(
                payload["last_refresh_boundary"]
            )
            scanner._scheduler_task = None
        except (KeyError, TypeError, ValueError, json.JSONDecodeError) as exc:
            raise ScannerStateRecoveryError(
                "Scanner runtime snapshot is invalid"
            ) from exc
        return True
...
class PersistentScannerService(ScannerService):
    ...
    def __init__(
        self,
        *args: Any,
        state_store: ScannerRuntimeStateStore,
        **kwargs: Any,
    ) -> None:
    ...
    async def _scheduler_loop(self) -> None:  # pragma: no cover
        try:
            while self._state is ScannerState.ON:
                now = self._clock.now()
                if (
                    self._next_full_scan_at is not None
                    and now >= self._next_full_scan_at
                ):
                    await self.full_scan()
                if (
                    self._next_refresh_at is not None
