...
export const Settings: React.FC = () => {
  ...
  // Local editable settings state to support "Unsaved Changes" warning!
  const [editableSettings, setEditableSettings] = useState<AppSettings>(settings);
  const [hasChanges, setHasChanges] = useState<boolean>(false);
  const [showRestoreModal, setShowRestoreModal] = useState<boolean>(false);
  const [saveSuccess, setSaveSuccess] = useState<boolean>(false);

  // Sync settings when global store settings change (e.g. on emergency stop)
  useEffect(() => {
    setEditableSettings(settings);
    setHasChanges(false);
  }, [settings]);

  // Track changes compared to parent settings
  const updateField = (
    ...
    setEditableSettings((prev) => {
      ...
      // Compare next vs parent settings
      const stringifiedNext = JSON.stringify(next);
      const stringifiedParent = JSON.stringify(settings);
      setHasChanges(stringifiedNext !== stringifiedParent);
  ...
  return (
    <div id="settings-page" className="flex flex-col gap-4 relative">
        ...
        <div className="bg-amber-950 border border-amber-900 rounded-lg p-3.5 flex items-center justify-between gap-3 text-xs font-mono animate-fade-in z-20">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="bg-zinc-800 hover:bg-zinc-700 text-zinc-300 px-3 py-1.5 rounded font-bold transition-colors"
            >
              Discard Changes
            </button>
            <button
      ...
      {/* Compact Settings Status Overview Grid */}
      <div className="bg-zinc-950 border border-zinc-900 rounded-lg p-3 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3.5 text-xs font-mono select-none">
        ...
        <div className="flex flex-col gap-1 border-r border-zinc-850/60 pr-2 last:border-0">
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            <span className={`w-2 h-2 rounded-full ${editableSettings.automation.autoScan ? "bg-emerald-500" : "bg-zinc-600"}`} />
            <span className="text-zinc-200 font-bold">{editableSettings.automation.autoScan ? "Active Scanner" : "Idle State"}</span>
          </div>
        </div>
      ...
      {/* Double Column Settings layout */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        
        {/* Left internal menu (3 cols) */}
        <div className="lg:col-span-3 flex flex-col gap-1 bg-zinc-900 border border-zinc-800 rounded-lg p-2 font-mono text-xs select-none h-fit">
          <h3 className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider p-2 border-b border-zinc-850 mb-1">
            Settings Menu
          </h3>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded text-left font-semibold transition-all ${
                  isActive
                    ? "bg-orange-600/10 text-orange-500 border-l-2 border-orange-500 font-bold"
          ...
          <div className="mt-4 pt-3 border-t border-zinc-850 p-2">
            <button
              ...
              className="w-full bg-zinc-950 hover:bg-rose-950/20 hover:text-rose-400 border border-zinc-800 text-zinc-500 py-2 rounded font-bold flex items-center justify-center gap-1.5 transition-all text-[11px]"
            >
              <RotateCcw size={12} />
        ...
        {/* Right configuration pane (9 cols) */}
        <div className="lg:col-span-9 bg-zinc-900 border border-zinc-800 rounded-lg p-5 font-mono text-xs flex flex-col justify-between">
          <div>
            ...
            {activeTab === "binance" && (
              <div className="space-y-4">
                {/* Security Disclaimer & Instructions */}
                <div className="bg-amber-950/15 border border-amber-900/40 p-4 rounded-lg leading-relaxed text-zinc-300 space-y-2 select-none">
                  ...
                  <p className="text-[11px]">
                    The terminal connects to the AstraForge FastAPI backend. Exchange credentials remain backend-only. Execution stays disabled until the Render service is explicitly configured and verified.
                  </p>
                ...
                {/* Multi-tier Connection States Display */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-2">
                  <div className="flex flex-col justify-between bg-zinc-950 p-3 rounded border border-zinc-850">
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      ...
                      <span className={`font-bold ${marketStatus === "Connected" ? "text-emerald-400" : marketStatus === "Degraded" ? "text-amber-400" : "text-rose-500"}`}>
                        {marketStatus === "Connected" ? "AVAILABLE" : marketStatus === "Degraded" ? "DEGRADED" : "UNAVAILABLE"}
                      </span>
                    </div>
                  </div>
            ...
            {activeTab === "rules" && (
              <div className="space-y-4">
                <div className="bg-zinc-950 p-2.5 rounded border border-zinc-850 leading-relaxed text-zinc-400">
                  <span className="text-amber-500 font-bold">Intraday Locked Timeframes Roles:</span>
                  <ul className="list-disc pl-4 space-y-1 mt-1 text-[11px] text-zinc-300">
                    <li><span className="font-bold text-white">1H (Primary trend):</span> Establishes general macro-direction and market regime.</li>
                    <li><span className="font-bold text-white">15M (Setup detection):</span> Locates high probability reversal or breakout trigger zones.</li>
                    <li><span className="font-bold text-white">5M (Entry trigger):</span> Identifies active candle volume and momentum confirmations.</li>
                  </ul>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  ...
                  <div className="flex items-center justify-between bg-zinc-950 p-3 rounded border border-zinc-850">
                    ...
                    <input
                      type="checkbox"
                      checked={editableSettings.rules.longEnabled}
                      onChange={(e) => updateField("rules", "longEnabled", e.target.checked)}
                      className="w-4 h-4 text-orange-600 bg-zinc-900 border-zinc-850 rounded focus:ring-0 cursor-pointer"
                    />
                  </div>

                  {/* Short Enabled toggle */}
                  <div className="flex items-center justify-between bg-zinc-950 p-3 rounded border border-zinc-850">
                    <span className="text-zinc-300">Allow Short Trades:</span>
                    <input
                      type="checkbox"
                      checked={editableSettings.rules.shortEnabled}
                      onChange={(e) => updateField("rules", "shortEnabled", e.target.checked)}
                      className="w-4 h-4 text-orange-600 bg-zinc-900 border-zinc-850 rounded focus:ring-0 cursor-pointer"
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between bg-zinc-950 p-3 rounded border border-zinc-850">
                    ...
                    <input
                      type="checkbox"
                      checked={editableSettings.rules.gradeBPlusWatchOnly}
                      onChange={(e) => updateField("rules", "gradeBPlusWatchOnly", e.target.checked)}
                      className="w-4 h-4 text-orange-600 bg-zinc-900 border-zinc-850 rounded focus:ring-0 cursor-pointer"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-1">
                  <div>
                    ...
                    <input
                      type="number"
                      value={editableSettings.rules.minimumConfidence}
                      onChange={(e) => updateField("rules", "minimumConfidence", parseInt(e.target.value) || 75)}
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      value={editableSettings.rules.minimumRiskReward}
                      onChange={(e) => updateField("rules", "minimumRiskReward", parseFloat(e.target.value) || 2.0)}
                      step="0.1"
                      className="w-full bg-zinc-950 border border-zinc-800 rounded px-2.5 py-1.5 text-white"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      value={editableSettings.rules.minimum24hVolume}
                      onChange={(e) => updateField("rules", "minimum24hVolume", parseInt(e.target.value) || 15000000)}
                      className="w-full bg-zinc-950 border border-zinc-800 rounded px-2.5 py-1.5 text-white"
                    />
                  </div>
                </div>
            ...
            {/* TAB CONTENT: RISK ENGINE */}
            {activeTab === "risk" && (
              <div className="space-y-4">
                <div className="bg-zinc-950 p-2.5 rounded border border-zinc-850 leading-relaxed text-zinc-400 text-[11px]">
                  <span className="text-rose-500 font-bold">CRITICAL DEVIATION BLOCK:</span> The Risk Engine is a hardened system module and cannot be disabled. Standard risk ceilings protect perpetual accounts from liquidations during systemic market black swans.
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-zinc-400 mb-1 uppercase font-bold text-[10px]">Risk Per Trade (%):</label>
                    <input
                      type="number"
                      value={editableSettings.risk.riskPerTradePercent}
                      onChange={(e) => updateField("risk", "riskPerTradePercent", parseFloat(e.target.value) || 1.0)}
                      step="0.1"
                      className="w-full bg-zinc-950 border border-zinc-800 rounded px-2.5 py-1.5 text-white"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-zinc-400 mb-1 uppercase font-bold text-[10px]">Daily Loss Limit (%):</label>
                    <input
                      type="number"
                      value={editableSettings.risk.dailyLossLimitPercent}
                      onChange={(e) => updateField("risk", "dailyLossLimitPercent", parseFloat(e.target.value) || 3.0)}
                      step="0.5"
                      className="w-full bg-zinc-950 border border-zinc-800 rounded px-2.5 py-1.5 text-white"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-zinc-400 mb-1 uppercase font-bold text-[10px]">Daily Profit Lock (%):</label>
                    <input
                      type="number"
                      value={editableSettings.risk.dailyProfitLockPercent}
                      onChange={(e) => updateField("risk", "dailyProfitLockPercent", parseFloat(e.target.value) || 5.0)}
                      step="0.5"
                      className="w-full bg-zinc-950 border border-zinc-800 rounded px-2.5 py-1.5 text-white"
                    />
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-1">
                  <div>
                    <label className="block text-zinc-400 mb-1 uppercase font-bold text-[10px]">Max Concurrent Open Trades:</label>
                    <input
                      type="number"
                      value={editableSettings.risk.maxOpenTrades}
                      onChange={(e) => updateField("risk", "maxOpenTrades", parseInt(e.target.value) || 4)}
                      className="w-full bg-zinc-950 border border-zinc-800 rounded px-2.5 py-1.5 text-white"
                    />
                  </div>

                  <div>
                    <label className="block text-zinc-400 mb-1 uppercase font-bold text-[10px]">Max Leverage Limit:</label>
                    <input
                      type="number"
                      value={editableSettings.risk.maxLeverage}
                      onChange={(e) => updateField("risk", "maxLeverage", parseInt(e.target.value) || 10)}
                      className="w-full bg-zinc-950 border border-zinc-800 rounded px-2.5 py-1.5 text-white"
                    />
                  </div>

                  <div>
                    <label className="block text-zinc-400 mb-1 uppercase font-bold text-[10px]">Per-Symbol Position Limit:</label>
                    <input
                      type="number"
                      value={editableSettings.risk.perSymbolTradeLimit}
                      onChange={(e) => updateField("risk", "perSymbolTradeLimit", parseInt(e.target.value) || 1)}
                      className="w-full bg-zinc-950 border border-zinc-800 rounded px-2.5 py-1.5 text-white"
                    />
                  </div>
                </div>
            ...
            {activeTab === "automation" && (
              <div className="space-y-4">
                <div className="bg-amber-950/15 border border-amber-900/40 p-3 rounded leading-relaxed text-zinc-300">
                  <span className="text-amber-500 font-bold flex items-center gap-1.5 mb-1">
                    <AlertTriangle size={15} /> Not Implemented
                  </span>
                  Backend engines (Scanner, Signal, Execution) are unavailable. Automation controls are disabled.
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* AutoScan */}
                  <div className="flex items-center justify-between bg-zinc-950 p-3 rounded border border-zinc-850 opacity-50">
                    <div>
                      <span className="text-zinc-200 font-bold block">Auto Scan Market:</span>
                      <span className="text-[10px] text-zinc-500 font-mono">Loop Binance API continuously</span>
                    </div>
                    <input
                      type="checkbox"
                      disabled={true}
                      checked={false}
                      className="w-4 h-4 text-orange-600 bg-zinc-900 border-zinc-850 rounded focus:ring-0 cursor-not-allowed"
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between bg-zinc-950 p-3 rounded border border-zinc-850 opacity-50">
                    <div>
                      <span className="text-zinc-200 font-bold block">Auto Signal Dispatch:</span>
                      <span className="text-[10px] text-zinc-500 font-mono">Push qualified alerts to interface</span>
                    </div>
                    <input
                      type="checkbox"
                      disabled={true}
                  ...
                  <div className="flex items-center justify-between bg-zinc-950 p-3 rounded border border-zinc-850 opacity-50">
                    <div>
                      <span className="text-rose-400 font-bold block">Auto Execution:</span>
                      <span className="text-[10px] text-zinc-500 font-mono">Skip confirmation popup</span>
                    </div>
                    <input
                      type="checkbox"
                      disabled={true}
                  ...
                  <div className="flex items-center justify-between bg-zinc-950 p-3 rounded border border-zinc-850 opacity-50">
                    <div>
                      <span className="text-zinc-200 font-bold block">Move SL to Breakeven:</span>
                      <span className="text-[10px] text-zinc-500 font-mono">Move stop loss when TP1 is hit</span>
                    </div>
                    <input
                      type="checkbox"
                      disabled={true}
            ...
            {activeTab === "notifications" && (
              <div className="space-y-4">
                <div className="bg-zinc-950 p-3 rounded border border-zinc-850 leading-relaxed text-zinc-400">
                  Receive instant telemetry messages on trade fills, stop hits, and risk limits warnings. Connect your personal Telegram channel.
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex items-center justify-between bg-zinc-950 p-3 rounded border border-zinc-850">
                    ...
                    <input
                      type="checkbox"
                      checked={editableSettings.notifications.telegramEnabled}
                      onChange={(e) => updateField("notifications", "telegramEnabled", e.target.checked)}
                      className="w-4 h-4 text-orange-600 bg-zinc-900 border-zinc-850 rounded focus:ring-0 cursor-pointer"
                    />
                  </div>
                  ...
                  {editableSettings.notifications.telegramEnabled && (
                    <div className="bg-zinc-950 p-3 rounded border border-zinc-850 flex items-center justify-between">
                      ...
                      <input
                        type="text"
                        placeholder="e.g. -10045920310"
                        value={editableSettings.notifications.telegramChatId}
                        onChange={(e) => updateField("notifications", "telegramChatId", e.target.value)}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-2">
                  {[
                    ...
                    <div key={notif.field} className="flex items-center justify-between bg-zinc-950 p-3 rounded border border-zinc-850">
                      ...
                      <input
                        type="checkbox"
                        checked={Boolean((editableSettings.notifications as unknown as Record<string, unknown>)[notif.field])}
                        onChange={(e) => updateField("notifications", notif.field, e.target.checked)}
                        className="w-4 h-4 text-orange-600 bg-zinc-900 border-zinc-850 rounded focus:ring-0 cursor-pointer"
                      />
                    </div>
                  ))}
            ...
            {activeTab === "system" && (
              <div className="space-y-4">
                <div className="bg-zinc-950 p-3 rounded border border-zinc-850 text-zinc-400 leading-relaxed">
                  Diagnostics for the React/Vite client and its connection to the AstraForge FastAPI service.
                </div>

                <div className="bg-zinc-950/60 rounded border border-zinc-850 p-3 space-y-2 text-[11px]">
                  <div className="flex justify-between pb-1.5 border-b border-zinc-850">
                    <span className="text-zinc-500">Framework version:</span>
                    <span className="text-zinc-200">React 19.0.1 (Vite & Tailwind v4)</span>
                  </div>
                  <div className="flex justify-between pb-1.5 border-b border-zinc-850">
                    <span className="text-zinc-500">API service layer:</span>
                    <span className="text-emerald-400 font-bold">Centralized FastAPI client</span>
                  </div>
                  <div className="flex justify-between pb-1.5 border-b border-zinc-850">
                    <span className="text-zinc-500">Backend Port Binding:</span>
                    <span className="text-zinc-400">VITE_API_BASE_URL (required at build time)</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-zinc-500">Diagnostics log retention:</span>
                    <span className="text-zinc-200">{editableSettings.system.logRetentionDays} Days</span>
                  </div>
                </div>
              </div>
            )}
          ...
          {/* Sticky save action inside card footer for clean desktop UI */}
          {!hasChanges && (
            <div className="pt-4 border-t border-zinc-800 text-zinc-500 text-[10px] text-right select-none">
              All settings synchronized. No unsaved changes.
            </div>
          )}
        </div>
      ...
      {showRestoreModal && (
        <div className="fixed inset-0 z-50 bg-black/75 flex items-center justify-center p-4 font-mono text-xs select-none">
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg max-w-sm w-full p-4 space-y-4">
            <div className="flex items-center gap-2 text-rose-500 font-bold uppercase">
              <RotateCcw size={16} />
              <span>Confirm System Restore</span>
            </div>
            <p className="text-zinc-300 leading-relaxed text-[11px]">
              Are you sure you want to restore the bot to its factory configuration? This will clear all custom risk parameters, session times, Telegram integrations, and saved API permissions.
            </p>
            <div className="flex gap-2">
              <button
                onClick={() => setShowRestoreModal(false)}
                className="flex-1 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 py-2 rounded font-bold"
              >
                Cancel
              </button>
              <button
                onClick={confirmRestoreDefaults}
                className="flex-1 bg-rose-600 hover:bg-rose-700 text-white py-2 rounded font-bold"
              >
